...
double Chebyshev(const int n, const int k){
    ...
    int i, j, ki;
    double chebyshev, dis;
#   pragma omp parallel for num_threads(thread_count) \
        default(none) shared(n, k, rebuiltCoord) private(i, j, ki, chebyshev, dis) reduction(+:chebyshevSum) schedule(dynamic, dynamic_count)
    ...
    for(i=0; i<n; i++)
        for(j= i + 1; j<n; j++){
            ...
            for(ki=0; ki<k; ki++){
                ...
                if(dis>chebyshev) chebyshev = dis;
            }
            
            chebyshevSum = chebyshevSum + chebyshev * 2;
...
double SumDistance(const int k, const int n, const int dim, double* coord, int* pivots){
    ...
    //对所有点进行遍历
#   pragma omp parallel for num_threads(thread_count) \
        default(none) shared(n, k, rebuiltCoord, dist, pivots) private(i, ki, pivoti) schedule(dynamic, dynamic_count)
    for(i=0; i<n; i++)
        //ki = 0, 1
        //新坐标的维数应该是k
        for(ki=0; ki<k; ki++){
